...
class BenchSuiteTest {
  ...
  // ── Live LLM Client Tests ──
  // Run with: mvn test -Dtest=BenchSuiteTest -Dbench.live=true

  @Nested
  class LiveLlmClientTests {
    ...
    void liveClient_runAllFixtures_generatesReport() {
      ...
      OpenAiCompatibleLlmClient liveClient = new OpenAiCompatibleLlmClient();
      BenchRunner runner = new BenchRunner(liveClient);
      List<BenchResult> results = runner.run();
      BenchRunner.AggregateStats stats = BenchRunner.aggregate(results);

      String report = BenchReportWriter.generateReport(results, stats, "live");

      assertThat(report).contains("MCP Bench Report — live");
      assertThat(report).contains("Summary");

      // Write report to file for inspection
      try {
        java.nio.file.Path outputPath = java.nio.file.Paths.get("target/bench-report.live.md");
        java.nio.file.Files.writeString(outputPath, report);
        System.out.println("Live bench report written to: " + outputPath.toAbsolutePath());
        System.out.println("Total cost: $" + String.format("%.4f", liveClient.getTotalCostUsd()));
      } catch (Exception e) {
        System.err.println("Failed to write live report: " + e.getMessage());
      }
    }
  }

  private static boolean isLiveBenchEnabled() {
    String liveFlag = System.getProperty("bench.live", "false");
    String apiKey = System.getenv("OPENAI_API_KEY");
    return "true".equalsIgnoreCase(liveFlag) && apiKey != null && !apiKey.isBlank();
  }
}
